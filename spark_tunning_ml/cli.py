...
def process_executors(sparkui, path_executors, id):
    """
    ...
    """
    logger.info(f"Processing executors for application {id}")
    executors = sparkui.get_executors(id)
...
def process_stages(sparkui, path_stages, id):
    """
    ...
    """
    logger.info(f"Processing stages for application {id}")
    stages = sparkui.get_stages(id)
...
def process_stage_task_detail(
    sparkui, path_stage_tasks_detail, id, stage, stage_attempt_id
):
    """
    ...
    """
    response_stage_tasks_detail = sparkui.get_task_list(id, stage, stage_attempt_id)
...
def process_stage_task_summary(
    sparkui, path_stage_tasks_summary, id, stage, stage_attempt_id
):
    """
    ...
    response_stage_tasks_summary = [
        sparkui.get_task_summary(id, stage, stage_attempt_id)
    ]
...
def process_stage(sparkui, path_stage, stages, id):
    """
    ...
    if data.check_empty_list(stages):
        for stage in stages:
            ...
            stage_response = sparkui.get_stage(id, stage)
            ...
            stage_attempt_id = sparkui.get_id_from_stage_attempts(stage_response)

            stages_attempts_ids[stage] = stage_attempt_id
...
def process_jobs(sparkui, path_jobs, id):
    """
    ...
    """
    logger.info(f"Processing jobs for application {id}")
    jobs = sparkui.get_jobs(id)
...
def process_environment(sparkui, path_environment, id):
    """
    ...
    """
    logger.info(f"Processing environment for application {id}")
    environment = [sparkui.get_environment(id)]
...
def process_application(id, sparkui):
    """
    ...
    paths = [
        ...
        f'{path_root}/{config.get("spark_ui_path_stage_tasks_summary")}',
    ...
    process_executors(sparkui, paths[0], id)

    stages = process_stages(sparkui, paths[1], id)

    stages_attempts_ids = process_stage(sparkui, paths[2], stages, id)

    for stage, stage_attempt_id in stages_attempts_ids.items():
        process_stage_task_detail(sparkui, paths[3], id, stage, stage_attempt_id)
        process_stage_task_summary(sparkui, paths[4], id, stage, stage_attempt_id)

    process_jobs(sparkui, paths[5], id)

    process_environment(sparkui, paths[6], id)
...
def main():
    ...
    parser.add_argument(
        "--base_url", required=True, help="The base URL for the requests."
    )
    ...
    applications_ids = sparkui.get_ids_from_applications(
        applications, config.get("internal_spark_ui_allow_not_complete_apps")
    )
    ...
    with ThreadPoolExecutor(
        ...
        [executor.submit(process_application, id, sparkui) for id in applications_ids]
